<article class="course-card" data-category="{{ $course['category'] }}" data-modality="{{ $course['modality'] }}" data-price="{{ $course['discountPrice'] }}">
    @php $hasDiscount = $course['discountPrice'] < $course['originalPrice']; @endphp

    {{-- Countdown timer bar (only when discounted) --}}
    @if($hasDiscount)
        ...
        <div>
            ...
            <div class="course-card__date-status">
                <div class="course-card__date">
                    <svg height="15" viewBox="0 0 24 24" fill="none" stroke="#704EFD" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
