...
export default function CourseCard({ course }: CourseCardProps) {
  ...
  return (
    <article className="bg-white rounded-[10px] border border-adipa-border shadow-card hover:shadow-card-hover hover:-translate-y-[10px] transition-all duration-500 max-w-[350px] mx-auto w-full flex flex-col h-full min-h-[330px]">
      {/* Countdown timer bar */}
      {hasDiscount && (
        <div className="flex justify-center items-center px-1 py-[6px] rounded-t-[10px] flex-shrink-0 bg-adipa-deep-purple">
      ...
      <div className={`relative aspect-video overflow-hidden flex-shrink-0 ${!hasDiscount ? "rounded-t-[10px]" : ""}`}>
        ...
        <div className="absolute z-[1] flex items-center flex-row-reverse gap-[3px] bottom-5 right-0 bg-white rounded-l-[5px] px-[10px] py-[2px] shadow-[0_4px_4px_0_rgba(0,0,0,0.39)]">
          ...
          <span className={`w-[7px] h-[7px] rounded-full inline-block flex-shrink-0 ${dotClass}${course.modality === "En Vivo" ? " animate-pulse-dot" : ""}`} />
        </div>
      </div>
      ...
      <div className="relative flex flex-col justify-between flex-1 pt-[25px] px-5 pb-5 min-h-[260px]">
        ...
        <div>
          ...
          <div className="flex items-center flex-wrap justify-between mb-5">
            <div className="flex items-center gap-1">
              <svg height="15" viewBox="0 0 24 24" fill="none" stroke="#704EFD" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                <line x1="16" y1="2" x2="16" y2="6" />
                <line x1="8" y1="2" x2="8" y2="6" />
                <line x1="3" y1="10" x2="21" y2="10" />
              </svg>
              <span className="text-xs text-muted">Inicio : {course.startDate}</span>
            </div>
            <div className="flex items-center gap-1">
        ...
        <div>
            ...
            {hasDiscount ? (
              <>
                <div className="flex items-center gap-[5px]">
                  <svg height="24" viewBox="0 0 24 24" fill="#ff4500" aria-hidden="true">
          ...
          <div className="flex items-center justify-between w-full">
            <button
              type="button"
              className="w-full bg-transparent text-adipa-purple text-[14px] font-medium font-poppins p-[10px] rounded-[5px] border border-adipa-purple cursor-pointer transition-colors duration-200 hover:bg-adipa-purple hover:text-white hover:border-adipa-purple"
            >
              Ver detalle +
            </button>
            <button
              type="button"
              className="flex-shrink-0 flex items-center justify-center min-w-[44px] min-h-[44px] w-[44px] bg-adipa-purple border border-adipa-purple rounded-[5px] ml-[10px] cursor-pointer transition-colors duration-200 hover:bg-adipa-cyan hover:border-adipa-cyan"
